...
import { showToast, formatPrice, icon, escapeHtml } from './portfolio-shared.js';
import {
...
function updateCartBadges() {
  ...
  ['cart-count', 'cart-count-menu', 'cart-count-mobile'].forEach((id) => {
    const el = $(id);
    if (el) el.textContent = count;
  });
}

function resolveLifeSkuId(item) {
...
function cartForApi() {
  return cart.map((item) => {
    const sku_id = resolveLifeSkuId(item);
    return { ...item, sku_id, id: sku_id };
  });
}
...
function validateForm() {
  ...
  const cartCheck = validateCartHasSku(cartForApi());
  if (!cartCheck.valid) {
...
function savePurchasedProtocolFromCheckout(orderResponse) {
  try {
    const pending = JSON.parse(localStorage.getItem('lifeProtocolPending') || 'null');
    const tierKey = pending?.tierKey
      || JSON.parse(localStorage.getItem('lifeProtocolSelectedTier') || 'null')?.tierKey
      || JSON.parse(localStorage.getItem('lifeProtocolSelectedTier') || 'null')?.tierName;
    const resultData = JSON.parse(sessionStorage.getItem('lifeProtocolResult') || 'null')
      || JSON.parse(localStorage.getItem('lifeProtocolResultPersistent') || 'null');
    if (!resultData?.tiers || !tierKey) return;

    const resolvedKey = resultData.tiers[tierKey]
      ? tierKey
      : (resultData.recommended_tier || 'optimal');
    const tier = resultData.tiers[resolvedKey];
    if (!tier) return;
    ...
    const payload = {
      ...
      recommended_tier: resultData.recommended_tier || resolvedKey
    };
    localStorage.setItem('lifeMyProtocol', JSON.stringify(payload));
    localStorage.removeItem('lifeProtocolPending');
  } catch (e) {
    console.warn('Could not save purchased protocol', e);
  }
}

function prefillFromProtocolLead() {
  try {
    const lead = JSON.parse(localStorage.getItem('lifeProtocolLead') || 'null');
    if (lead?.email && $('email') && !$('email').value) {
      $('email').value = lead.email;
    }
    if (lead?.name) {
      const parts = lead.name.trim().split(/\s+/);
      if ($('first-name') && !$('first-name').value) {
        $('first-name').value = parts[0] || '';
      }
      if ($('last-name') && parts.length > 1 && !$('last-name').value) {
...
async function submitOrder(e) {
  ...
  try {
    ...
    savePurchasedProtocolFromCheckout(data.order);
    localStorage.removeItem(CART_KEY);
    sessionStorage.setItem(LAST_ORDER_KEY, JSON.stringify(data.order));
    window.location.href = `life-order-success.html?id=${encodeURIComponent(data.order.id)}`;
  } catch (err) {
...
function init() {
  document.body.classList.add('pf-body--checkout', 'life-checkout-page');
  cart = getCart();
  prefillFromProtocolLead();
  renderCart();
