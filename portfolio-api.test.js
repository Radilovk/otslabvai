...
describe('Portfolio stock-check', () => {
  const catalogChunk = [{
    group_id: '100',
    name: 'Test Protein',
    brand: 'TestBrand',
  ...
  test('POST /portfolio/stock-check returns KV availability without full validate', async () => {
    const request = new Request('https://example.com/portfolio/stock-check', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ skus: ['1', '2', 'missing'] })
    });
    const res = await handlePortfolioRoute(request, makeEnv(), new URL(request.url));
    expect(res.status).toBe(200);
    const data = await res.json();
    expect(data.all_available).toBe(false);
    expect(data.items).toEqual([
      { sku_id: '1', available: true, name: 'Test Protein – 1 кг – Шоколад' },
      { sku_id: '2', available: false, name: 'Test Protein – 1 кг – Ванилия' },
      { sku_id: 'missing', available: false, name: undefined }
    ]);
  });
});
