...
### 2026-07-10 — Импорт на продукти от Portfolio каталога към index/life (с AI подбор)

**Задача:** Продукти от Portfolio B2B каталога (Fitness1 API) да могат да се интегрират автоматично или чрез AI подбор в проектите „Да Отслабна" (отслабване) и Life Protocols (антиейджинг), по напълно хомогенен начин спрямо съществуващите продукти.

**Решение:**
- **portfolio-import.js (нов)** — конвертира каталожни групи към схемата `public_data`/`system_data` (име, марка, цена = мин. retail на наличните варианти, варианти с SKU/EAN, изображение, етикет, опаковка); сливане в page content с дедупликация по `system_data.portfolio.group_id`; опресняване само на търговските полета (ръчните текстове се пазят); AI prompt за подбор по цел на проекта.
- **portfolio-api.js** — нови експорти `getPortfolioMeta()` и `loadPortfolioGroupsByIds()` (ефективно четене на chunk-ове).
- **worker.js** — нови маршрути: `GET /portfolio/import/preview`, `POST /portfolio/import/ai-select` (ползва конфигурирания AI доставчик от ai_settings), `POST /portfolio/import/apply` (директен сървърен импорт + GitHub sync), `POST /portfolio/import/refresh`. `POST /portfolio/sync` сега автоматично опреснява цените/наличностите на импортираните продукти в двата проекта.
- **admin.html/admin.js/admin.css** — бутон „🔗 Импорт от Portfolio (AI)" в редактора на категория: модал с търсене/филтри, „🤖 AI подбор" (с причина, слоган и цели за всеки продукт) и опционално „AI обогатяване след импорт" през съществуващия AI Асистент. Portfolio метаданните се пренасят през `dataset.pfMeta`, за да не се губят при сериализация на редактора.
...
### 2026-07-10 (3) — Съвършено оформяне на импортираните продукти + Life UX/checkout преработка + E2E

**Част 1 — данни при импорт:** `htmlToStructuredContent()` в portfolio-import.js разбива HTML описанията от Fitness1 на: кратко описание (1-ви параграф, срязан на изречение), секция „За продукта" (заглавие от `<h2>`, останалите параграфи) и ползи от `<li>` (title/text). Продуктовите страници (product.js + life-product.js) рендират описанията като параграфи (`renderTextParagraphs`). Картите в life/index: fallback на слогана от описанието, вариантни цени „от X €", без NaN при null цена.

**Част 2 — Life checkout UX (life-checkout.html):** тост нотификации вместо alert/confirm; защита от двойно изпращане + „⏳ Изпращане..."; блокиран бутон и празно състояние при празна количка (беше възможно изпращане на празна поръчка!); inline промо съобщения + „✕ Премахни" за приложен код; подсказка „Още X € до безплатна доставка"; inline маркиране на липсващ куриерски офис + scroll към първата грешка; success модал изчиства количката на екрана и при затваряне връща към life.html.

**Част 3 — E2E (`e2e/life-e2e.mjs`, npm run test:life-e2e):** пълният път вход→карта→продукт (варианти)→количка→промо→валидации→Speedy офис (postMessage симулация)→поръчка→потвърждение + разклонения (празна количка, до адрес, мобилен изглед). 36 проверки. Фикстурата минава през portfolioGroupToSiteProduct — тества целия импорт pipeline. Стъб бекендът е на localhost:8080/backend (изискване на API_URL логиката).
