...
"ОТСЛАБВАНЕ" е онлайн магазин, който визуализира продукти чрез ключа `page_content` в KV (достъпно през `/page_content.json`) и съхранява поръчките локално или в Cloudflare KV. Основните HTML страници се намират в корена на проекта.

## ⚠️ Важно: Известен Проблем с Съдържанието

Ако забележите, че страницата накратко показва едно съдържание, а след това се сменя на друго (например от "ОТСЛАБВАНЕ" на "BIOCODE"), това означава че Cloudflare KV съдържа остарели данни.

**Бързо Решение:**
```bash
# Актуализирайте KV съдържанието с правилните данни
npm run update-kv
```

За подробности вижте [docs/FIX-CONTENT-MISMATCH.md](docs/FIX-CONTENT-MISMATCH.md)
