...
const PARTICLE_COLORS = ['rgba(228,0,124,', 'rgba(180,0,90,', 'rgba(0,169,157,'];
        ...
        for (let i = 0; i < particles.length; i++) {
            ...
            for (let j = i + 1; j < particles.length; j++) {
                ...
                if (dist < LINK_DIST) {
                    ...
                    ctx.moveTo(p.x, p.y);
                    ctx.lineTo(q.x, q.y);
                    ctx.strokeStyle = `rgba(228,0,124,${alpha})`;
                    ctx.lineWidth = 0.6;
                    ctx.stroke();
                }
            }
...
function initCard3DTilt() {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        if (!card) {
            // Reset the previously tilted card only
            if (_tiltedCard) {
                _tiltedCard.classList.remove('tilting');
                _tiltedCard.style.transform = '';
                _tiltedCard = null;
            }
            return;
        }
